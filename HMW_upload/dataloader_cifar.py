...
class cifar_dataset(Dataset): 
    def __init__(self, dataset, sample_ratio, r, noise_mode, root_dir, data_dir, transform, mode, noise_file='', pred=[], probability=[], log=''):
        ...
        if dataset == 'cifar10':
            root_dir = data_dir + '/cifar-10-batches-py'
            num_class =10         
        else:
            root_dir = data_dir + '/cifar-100-python'
            num_class =100
    ...
    def __getitem__(self, index):
        if self.mode=='labeled':
            ...
            return img1, img2, img3, img4,  np.int64(target), prob, index
            ...
            return img, np.int64(target), index
...
class cifar_dataloader():  
    def __init__(self, dataset, r, noise_mode, batch_size, num_workers, root_dir, data_dir, log, noise_file=''):
    ...
    def run(self, sample_ratio, mode, pred=[], prob=[]):
        if mode=='warmup':
            all_dataset = cifar_dataset(dataset=self.dataset, sample_ratio= sample_ratio, noise_mode=self.noise_mode, r=self.r, root_dir=self.root_dir, data_dir=self.data_dir, transform=self.transforms["warmup"], mode="all",noise_file=self.noise_file)
            ...
            labeled_dataset = cifar_dataset(dataset=self.dataset, sample_ratio= sample_ratio, noise_mode=self.noise_mode, r=self.r, root_dir=self.root_dir, data_dir=self.data_dir, transform=self.transforms["labeled"], mode="labeled", noise_file=self.noise_file, pred=pred, probability=prob,log=self.log)
            ...
            unlabeled_dataset = cifar_dataset(dataset=self.dataset, sample_ratio= sample_ratio, noise_mode=self.noise_mode, r=self.r, root_dir=self.root_dir, data_dir=self.data_dir, transform=self.transforms["unlabeled"], mode="unlabeled", noise_file=self.noise_file, pred=pred)
            ...
            test_dataset = cifar_dataset(dataset=self.dataset, sample_ratio= sample_ratio, noise_mode=self.noise_mode, r=self.r, root_dir=self.root_dir, data_dir=self.data_dir, transform=self.transform_test, mode='test')
            ...
            eval_dataset = cifar_dataset(dataset=self.dataset, sample_ratio= sample_ratio, noise_mode=self.noise_mode, r=self.r, root_dir=self.root_dir, data_dir=self.data_dir, transform=self.transform_test, mode='all', noise_file=self.noise_file)
