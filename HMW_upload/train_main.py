...
parser.add_argument('--method', default='hmw+uc', type=str, help='hmw+cce, hmw+sce, hmw+uc, cce, sce, uc')
...
parser.add_argument('--if_tkum', default=1, type=float, help='if using TKUM')
    ...
    if '+' in args.method:
        if 'hmw' in args.method:
            ...
            weighting = NaturalDistanceWeighting(num_classes=args.num_class, feat_dim=512,
                                                 ...
                                                 if_tkum=args.if_tkum, if_anneal=args.if_anneal, if_spherical = args.if_spherical)
        else:
            assert 1 == 0
